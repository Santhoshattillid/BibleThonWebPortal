...
namespace AlbaDL
{
    public class BillingAddress
    {
        ...
        /// <summary>
        /// Getting list of billing addresses from GP
        /// </summary>
            ...
            if (xmlList != null)
                foreach (XmlNode xnParent in xmlList)
                {
                    ...
                    if (xmlNodeList != null)
                        ...
                        {
                            if (xn["CUSTNMBR"] != null && xn["ADDRESS1"] != null && xn["ADDRESS2"] != null &&
                                xn["ADDRESS3"] != null)
                            {
        ...
        /// <summary>
        /// Getting customer billing address for the filtered customer
        /// </summary>
        /// <param name="connString"></param>
        /// <param name="customerName"></param>
        /// <param name="telephone"></param>
        /// <returns></returns>
        public IEnumerable<BillingAddress> GetCustomerDetails(string connString, string customerName, string telephone)
        {
            var customeDetails = new List<BillingAddress>();
            var customerDetails = new EConnectModel().GetCustomerDetails(connString);
            var customerDoc = new XmlDocument();
            ...
            if (xmlList != null)
                ...
                {
                    bool isRecordFound = xn["CUSTNAME"] != null &&
                                         xn["CUSTNAME"].InnerText.ToLower().StartsWith(customerName.ToLower());
                    if (!string.IsNullOrEmpty(telephone))
                    {
                        if (!isRecordFound)
                            isRecordFound = xn["PHONE1"] != null &&
                                            xn["PHONE1"].InnerText.ToLower().Trim().StartsWith(
                                                telephone.ToLower().Trim());
                        if (!isRecordFound)
                        {
                            ...
                            if (xmlNodeList != null)
                                ...
                                {
                                    isRecordFound = xnBillingAddress["PHONE1"] != null &&
                                                    xnBillingAddress["PHONE1"].InnerText.ToLower().Trim().StartsWith(
                                                        telephone.ToLower().Trim());
                    ...
                    if (isRecordFound)
                    {
                        if (xn["CUSTNMBR"] != null && !string.IsNullOrEmpty(xn["CUSTNMBR"].InnerText))
                        {
                            ...
                            DateTime expireDate;
                            if (DateTime.TryParse(xn["CCRDXPDT"].InnerText, out expireDate))
                                billingAddr.CreditCardExpireDate = expireDate.ToString("MMyy");
        ...
        /// <summary>
        /// To get list of billing addresses from GP
        /// </summary>
        /// <param name="connString"></param>
        /// <returns></returns>
