...
/// </summary>
public class AlerePatientLookupCustomFilterCol : GridBoundColumn
{    
    ...
    {
        if (this.UniqueName != "DOB")
        {
            ((RadComboBox)o).DataTextField = this.DataField;
            ((RadComboBox)o).DataValueField = this.DataField;
            ((RadComboBox)o).DataSource = GetDataTable("SELECT DISTINCT TOP 1000  " + this.UniqueName + " FROM AS_WF_VW_AlerePatientLookup WHERE " + this.UniqueName + " LIKE '" + e.Text + "%'");
        ...
        else
        {
            ((RadComboBox)o).DataTextField = this.DataField;
            ((RadComboBox)o).DataValueField = this.DataField;
            ((RadComboBox)o).DataSource = GetDataTable("SELECT DISTINCT TOP 1000  " + this.UniqueName + " FROM AS_WF_VW_AlerePatientLookup WHERE CONVERT(VARCHAR(25), " + this.UniqueName + ", 120) LIKE '" + e.Text + "%'");
            ((RadComboBox)o).DataBind();
        }
    }
